...
impl<'tp> TorrentProcessor<'tp> {
    ...
    pub fn add_torrent_to_watchlist(mut self, argument: String) -> Result<String, String> {
        let mut err_str = "Regez format error. Use: addtowatchlist: <regex>";
        if let Ok(rgx) = Regex::new(argument.as_str()) {
            self.torrent_match_regex_list.push(rgx);
            self.options.add_dl_regex(argument);
